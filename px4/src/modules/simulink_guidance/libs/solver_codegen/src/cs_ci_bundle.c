...
csn* cs_chol(const cs* A, const css* S) {
    ...
    for (k = 0; k < n; k++) { /* compute L(k,:) for L*L' = C */
        ...
        /* --- Compute L(k,k) ----------------------------------------------- */
        if (CS_REAL(d) <= 0 || fabs(CS_IMAG(d)) >= DBL_EPSILON) {
...
#else
        if (!cs_entry(T, (CS_INT)i, (CS_INT)j, x + xi * (double _Complex)(I))) {
#endif
#else
...
#else
    return ((fabs(creal(lhs) - creal(rhs)) < DBL_EPSILON) & (fabs(cimag(lhs) - cimag(rhs)) < DBL_EPSILON)) ? 1 : 0;
#endif
}
